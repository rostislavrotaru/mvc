...
{
    use Spherus\HttpContext\HttpContext, Spherus\HttpContext\Session, Spherus\Parsers\IpFilterParser, Spherus\Interfaces\IModule, Spherus\Routing\RouteManager;
    ...
    class Workbench
    {
         ...
         *
         * @param
         *            Spherus\Core\ControllerBase The controller object to set
         */
        ...
        public static function LoadModules ()
        {
            ...
            if (isset($moduleDirectories)) {
                foreach ($moduleDirectories as $directory) {
                    ...
                    Check::FileIsReadable(
                            MODULES . $directory . SEPARATOR . 'module.php');
        ...
        public static function LoadController ()
        {
            ...
            if (isset($moduleObject)) {
                $fileName = MODULES . $parsedUrl->getModuleName() . SEPARATOR .
                         'controllers' . SEPARATOR .
                         $parsedUrl->getControllerName() . '.php';
                if (file_exists($fileName)) {
                    if (is_readable($fileName)) {
                        require ($fileName);
                        $controllerName = $moduleObject->GetNamespaceName() .
                                 '\\' . $parsedUrl->getControllerName() .
                                 'Controller';
                        ...
                        throw new SpherusException(
                                sprintf(EXCEPTION_FILE_NOT_READABLE, $fileName));
                    }
                } else {
                    unset($moduleObject);
                    throw new SpherusException(
                            sprintf(EXCEPTION_CONTROLLER_NOT_FOUND, 
                                    $parsedUrl->getControllerName()));
                }
            } else {
                unset($moduleObject);
                throw new SpherusException(
                        sprintf(EXCEPTION_MODULE_NOT_FOUND, 
                                $parsedUrl->getModuleName()));
            }
        }
        ...
        public static function LoadView ()
        {
            ...
            if (method_exists(Workbench::getCurrentController(), $action)) {
                call_user_func_array(
                        array(
                                Workbench::getCurrentController(),
                                $action
                        ), HttpContext::getParsedUrl()->getParameters());
                Workbench::getCurrentController()->AfterLoad();
            } else {
                throw new SpherusException(
                        sprintf(EXCEPTION_NO_CONTROLLER_ACTION_METHOD, $action, 
                                HttpContext::getParsedUrl()->getController(), 
                                HttpContext::getParsedUrl()->getModuleName()));
            }
            
            if (! in_array($action, self::$currentController->noViewControllers)) {
                $fileName = MODULES .
                         HttpContext::getParsedUrl()->getModuleName() . SEPARATOR .
                         'views' . SEPARATOR .
                         HttpContext::getParsedUrl()->getControllerName() .
                         SEPARATOR . $action . '.php';
                ...
                if (isset(self::$currentController->layout)) {
                    ...
                    // module
                    $layoutFile = self::$currentTheme->getLayoutsPath() .
                             SEPARATOR . self::$currentController->layout .
                             '.php';
                    ...
                    if (file_exists($layoutFile)) {
                        ...
                        $moduleThemeFile = MODULES .
                                 HttpContext::getParsedUrl()->getModuleName() .
                                 SEPARATOR . 'themes' . SEPARATOR .
                                 self::$currentTheme->getName() . SEPARATOR .
                                 'theme.php';
                        Check::FileIsReadable($moduleThemeFile);
                        require ($moduleThemeFile);
                        $moduleThemeName = self::getCurrentModule()->GetNamespaceName() .
                                 '\\Themes\\' . self::$currentTheme->getName() .
                                 'Theme';
                        ...
                        $layoutFile = $moduleThemeObject->getLayoutsPath() .
                                 SEPARATOR . self::$currentController->layout .
                                 '.php';
                        if (file_exists($layoutFile)) {
                            Check::FileIsReadable($layoutFile);
                            ...
                            throw new SpherusException(
                                    sprintf(EXCEPTION_LAYOUT_NOT_FOUND, 
                                            self::$currentController->layout));
                        }
        ...
        public static function LoadTheme ()
        {
            ...
            $currentThemeFilePath = THEMES . $currentThemeName . SEPARATOR .
                     'theme.php';
         ...
         *            The controller object to parse.
         *            
         ...
         *
         * @param Spherus\Interfaces\Imodule $module
         *            The module to add
